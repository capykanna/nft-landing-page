// METAMASK CONNECTION
const TIMEOUT = 1000;
const COLLECTION_NAME = 'CapyKanna';
let editions = [];
let dots = 1;
...
function updateStatusText(isOwner, checking) {
  ...
  if(checking) {
    if(isOwner) {
      statusText.innerText = `You do own ${COLLECTION_NAME}!! Welcome to the Jungle!${renderDots(dots)}`;
    } else {
      statusText.innerText = `Checking to see if you own any ${COLLECTION_NAME} ${renderDots(dots)}`;
    }
  } else {
    if(isOwner) {
      statusText.innerText = `You are part of the ${editions.length} ${COLLECTION_NAME} Jungle!!`;
    } else {
